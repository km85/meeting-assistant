...
# Make knowledge_base optional (saves ~5GB CUDA deps in Docker)
try:
...
class LLMHandler:
    """Generate responses using dual-model approach: fast for real-time, deep for recap."""
...
Language: Respond in the same language as the user's command or the meeting transcript."""
    ...
    FAST_MODEL = "gpt-4o-mini"      # For /assist, /say (real-time, low latency)
    DEEP_MODEL = "gpt-5.5"          # For /recap, /followup (deep analysis, best quality)

    @staticmethod
    async def generate_recap(session: Session) -> str:
        """Generate meeting recap from transcript. DEEP model for complex analysis."""
...
Keep it executive-friendly and actionable."""
        ...
        # Use DEEP model for recap
        return await LLMHandler._call_llm(prompt, session.session_id, model=LLMHandler.DEEP_MODEL)
    
    @staticmethod
    async def generate_say(session: Session) -> str:
        """Generate suggested answer to latest question. FAST model for real-time."""
        ...
        project_context = ""
        if session.active_project_id and knowledge_base:
...
Use tradeoff framing when appropriate."""
        ...
        # Use FAST model for real-time say
        return await LLMHandler._call_llm(prompt, session.session_id, model=LLMHandler.FAST_MODEL)
    
    @staticmethod
    async def generate_followup(session: Session) -> str:
        """Generate follow-up questions. DEEP model for complex analysis."""
...
- Focus on next steps and accountability"""
        ...
        # Use DEEP model for followup
        return await LLMHandler._call_llm(prompt, session.session_id, model=LLMHandler.DEEP_MODEL)
    
    @staticmethod
    async def generate_assist(session: Session) -> str:
        """Generate general real-time assistance. FAST model for low latency."""
        transcript = session.get_recent_transcript(minutes=10)
...
Keep it practical and actionable."""
        ...
        # Use FAST model for real-time assist
        return await LLMHandler._call_llm(prompt, session.session_id, model=LLMHandler.FAST_MODEL)
...
_(Note: Running in mock mode - connect OpenAI API key for real responses)_"""
    ...
    @staticmethod
    async def _call_llm(prompt: str, session_id: str, model: str = None) -> str:
        ...
        if settings.openai_api_key and len(settings.openai_api_key) > 50:
            return await LLMHandler._call_openai(prompt, session_id, max_retries, model)
    ...
    async def _call_kimi(prompt: str, session_id: str, max_retries: int) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                log_audit("llm_response_generated", session_id, {"latency": latency, "provider": "kimi", "model": settings.kimi_model})
    ...
    @staticmethod
    async def _call_openai(prompt: str, session_id: str, max_retries: int, model: str) -> str:
        """Call OpenAI API."""
        import openai
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                latency = time.time() - start_time
                log_audit("llm_response_generated", session_id, {"latency": latency, "provider": "openai", "model": model})
                ...
                if attempt == max_retries:
                    log_audit("llm_error", session_id, {"error": str(e), "provider": "openai", "model": model})
                    return f"Error generating response: {str(e)}"
                await asyncio.sleep(1)
